                                                   ...
                                                   gpointer u_data)
{
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next)
    {
        ...
        // 检查对应source_id的polygon是否为空，如果为空，则从文件中读取
        if (g_source_info[source_id].g_ploygon.empty())
        {
            // 获取画面的宽度和高度，注意这里不能使用source_frame_width，因为进入管道之后，画面的宽度和高度会发生变化
            guint width = frame_meta->pipeline_width;
            guint height = frame_meta->pipeline_height;
            readPoints("./configs/ds_yolo_tracker_config/polygon_" + std::to_string(source_id) + ".txt", g_source_info[source_id].g_ploygon, width, height);
            g_print("read polygon_%d.txt success!, frame height = %d, width = %d \r \n", source_id, height, width);
        }
        ...
        for (l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next)
        {
            ...
            //  如果中心点在多边形内
            if (isInside(g_source_info[source_id].g_ploygon, p))
            {
                // 更改检测框的颜色为红色
        ...
        // 绘制多边形
        guint line_num = g_source_info[source_id].g_ploygon.size();
        display_meta->num_lines = line_num;

        for (guint i = 0; i < line_num; i++)
        {
            NvOSD_LineParams *line_params = &display_meta->line_params[i];
            line_params->x1 = g_source_info[source_id].g_ploygon[i % line_num].x;
            line_params->y1 = g_source_info[source_id].g_ploygon[i].y;
            line_params->x2 = g_source_info[source_id].g_ploygon[(i + 1) % line_num].x;
            line_params->y2 = g_source_info[source_id].g_ploygon[(i + 1) % line_num].y;
        ...
        offset = snprintf(txt_params->display_text, MAX_DISPLAY_LEN, "Person Count = %d ", g_source_info[source_id].g_person_ids.size());
        snprintf(txt_params->display_text + offset, MAX_DISPLAY_LEN - offset, "Vehicle Count = %d ", g_source_info[source_id].g_vehicle_ids.size());
...
int main(int argc, char *argv[])
{
    GMainLoop *loop = NULL;
    // 创建各种元素
    GstElement *pipeline = NULL, *source = NULL, *source_1 = NULL, *streammux = NULL, *pgie = NULL, *nvvidconv = NULL,
    ...
    source_1 = create_source_bin(1, argv[2]); // 创建source_bin元素， 用于从文件中读取视频
